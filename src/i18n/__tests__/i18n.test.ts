...
describe("the locale set", () => {
  ...
  it("defines every English key in every other locale", () => {
    ...
    for (const lang of SUPPORTED_LANGUAGES) {
      ...
      expect({ lang, missing: base.filter((k) => !keys.includes(k)) }).toEqual({
        lang,
        missing: [],
      });
    }
...
describe("t", () => {
  it("returns the string for the active language", () => {
    speak("de");
    expect(t("settingsTitle")).toBe("Optionen");
  });

  it("interpolates named parameters", () => {
    expect(t("versionLabel", { version: "1.2.3" })).toBe("App version 1.2.3");
  });
  ...
  it("leaves an unknown parameter placeholder alone rather than erasing it", () => {
    expect(t("versionLabel", { other: "x" })).toBe("App version {version}");
  });
